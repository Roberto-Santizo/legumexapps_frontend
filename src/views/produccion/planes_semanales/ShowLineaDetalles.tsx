...
import { getTasksByLineId, TaskProduction as TaskProductionType } from "@/api/WeeklyProductionPlanAPI";
...
export default function ShowLineaDetalles() {
    ...
    const [selectedTask, setSelectedTask] = useState<TaskProductionType>({} as TaskProductionType);
    ...
    if (tasks) return (
        <div>
            ...
            {tasks.map(task => (
                <TaskProduction key={task.id} task={task} setSelectedTask={setSelectedTask} setModalCierre={setModalCierre} modalCierre={modalCierre} setModalRendimiento={setModalRendimiento}/>
            ))}
