...
import { PlannerFincasFilters } from "../hooks/usePlannerFincasFilters";
import React, { Dispatch, SetStateAction, useState } from "react";

interface Props {
    isOpen: boolean;
    setIsOpen: Dispatch<SetStateAction<boolean>>;
    currentFilters: PlannerFincasFilters;
    onApply: (filters: PlannerFincasFilters) => void;
    resetFilters: () => void;
}

export default function IndexFilters({ isOpen, setIsOpen, currentFilters, onApply, resetFilters }: Props) {
    const [localFilters, setLocalFilters] = useState<PlannerFincasFilters>(currentFilters);
    ...
    const handleResetFilters = () => {
        resetFilters();
        setLocalFilters({ finca: '', year: '', week: '' });
        setIsOpen(false);
    }
            ...
            >
                ...
                <div className="flex-1 overflow-y-auto max-h-screen p-2 space-y-4">
                    <div>
                        ...
                        <input type="text" name="finca" className="w-full border p-2 rounded" placeholder="Finca"
                            onChange={handleChange} value={localFilters.finca} autoComplete="off"
                        />
                    </div>

                    <div>
                        <label className="block text-sm font-medium">Semana</label>
                        <input type="text" name="week" className="w-full border p-2 rounded" placeholder="Semana"
                    ...
                    <div>
                        ...
                        <input type="number" name="year" className="w-full border p-2 rounded" placeholder="Año"
                            onChange={handleChange} value={localFilters.year} autoComplete="off"
                        />
                    </div>
                </div>
