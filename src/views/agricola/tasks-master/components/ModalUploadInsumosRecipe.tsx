...
import { uploadTasksGuidelinesInsumosRecipe } from "../api/TasksMasterAPI";
import Modal from "@/components/Modal";
import Spinner from "@/components/utilities-components/Spinner";

export default function ModalUploadInsumosRecipe() {
    const queryParams = new URLSearchParams(location.search);
    const flag = queryParams.get('uploadInsumosRecipe')!;
    ...
    const { mutate, isPending } = useMutation({
        mutationFn: uploadTasksGuidelinesInsumosRecipe,
        onError: (error) => {
    ...
    const handleCloseModal = () => {
        setSearchParams((searchParams) => {
            searchParams.delete('uploadInsumosRecipe');
            return searchParams;
        })
    }
    ...
    return (
        <Modal modal={show} closeModal={() => handleCloseModal()} title="Carga de Receta de Insumos" >
            <div className="p-5">
