import { uploadDraftTasksProduction } from "@/api/DraftWeeklyProductionPlanAPI";
...
export default function ModalUploadDraftTasksProduction() {
    const [_, setSearchParams] = useSearchParams();
    const [file, setFile] = useState<File[] | null>(null);
    const queryParams = new URLSearchParams(location.search);
    const params = useParams<{ id: string }>();
    const flag = queryParams.get('uploadTasks')!;
    const id = params.id!;
    ...
    const { mutate, isPending } = useMutation({
        mutationFn: (file: File) => uploadDraftTasksProduction(id, file),
        onError: (error) => {
    ...
    const handleCloseModal = () => {
        setSearchParams((searchParams) => {
            searchParams.delete('uploadTasks');
            return searchParams;
        })
    }
    ...
    return (
        <Modal modal={show} closeModal={() => handleCloseModal()} title="Carga Draft De Tareas" >
            <div className="p-5">
